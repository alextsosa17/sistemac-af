...
$CALIBordenes = explode(',', $calibracion_ordenes); //Los permisos para cada boton de Acciones.
          ...
          <div class="box">
            <div class="box-header">
                ...
                <?php if ($total_tabla > 0): ?>
                  <div class="box-tools">
                      <form action="<?= base_url('calibraciones_ordenes') ?>" method="POST" id="searchList">
                      ...
                      <?php foreach ($ordenes as $orden): ?>
                        <tr>
                          <td class="text-center">
                            <?php if (is_null($orden->ord_repa)): ?>
                              <p class="label label-primary" style="color:white; font-size:14px;">
                            <?php else: ?>
                              <p class="label label-warning" style="color:white; font-size:14px;">
                            <?php endif; ?>
                            <?=$orden->id?></p></td>
                          ...
                          <td class="text-center"><?= ($orden->dominio == NULL) ? 'A designar' : $orden->dominio; ?></td>
                          ...
                          <td>
                            <?php if (($CALIBordenes[0] == 1) && ($orden->tipo_orden == 30 || $orden->tipo_orden == 40 || $orden->tipo_orden == 51 || $orden->tipo_orden == 60 || $orden->tipo_orden == 61)): ?>
                              <a data-toggle="tooltip" title="Ver Detalle" href="<?= base_url().'verCalib/'.$orden->id; ?>"><i class="fa fa-info-circle"></i>&nbsp;&nbsp;&nbsp;</a>
                            <?php endif; ?>

                            <?php if (($CALIBordenes[1] == 1) && ($orden->tipo_orden == 30 || $orden->tipo_orden == 40 || $orden->tipo_orden == 51 || $orden->tipo_orden == 60 || $orden->tipo_orden == 61)): ?>
                              <a data-toggle="tooltip" title="Editar" href="<?= base_url().'editOldCalib/'.$orden->id; ?>"><i class="fa fa-pencil"></i>&nbsp;&nbsp;&nbsp;</a>
                            <?php endif; ?>

                            <?php if (($CALIBordenes[2] == 1) && ($orden->tipo_orden == 30 || $orden->tipo_orden == 40 || $orden->tipo_orden == 51 || $orden->tipo_orden == 60 || $orden->tipo_orden == 61)): ?>
                            ...
                            <?php if (($CALIBordenes[3] == 1) && ($orden->tipo_orden == 60 || $orden->tipo_orden == 61)): ?>
                              <a data-toggle="tooltip" title="Orden pendiente" href="<?= base_url().'espera/'.$orden->id; ?>"><i class="fa fa-arrow-circle-right"></i>&nbsp;&nbsp;&nbsp;</a>
                            <?php endif; ?>

                            <?php if (($CALIBordenes[4] == 1) && ($orden->tipo_orden == 51)): ?>
...
<script type="text/javascript">
    jQuery(document).ready(function(){
        jQuery('ul.pagination li a').click(function (e) {
            ...
            jQuery("#searchList").attr("action", baseURL + "calibraciones_ordenes/" + value);
